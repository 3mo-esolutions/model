...
export class DataGrid<TData, TDetailsElement extends Element | undefined = undefined> extends Component {
	...
	@property({ type: Array }) columnsDefinitions = new Array<ColumnDefinition<TData>>()
	...
	protected override firstUpdated(props: PropertyValues) {
		...
		this.columnsDefinitions = this.columns
	...
	get columns(): Array<ColumnDefinition<TData, KeyPathOf<TData>>> {
		return this.columnsDefinitions.length > 0 ? this.columnsDefinitions
			: this.elementExtractedColumns.length > 0 ? this.elementExtractedColumns
				: this.autoGeneratedColumnDefinitions
	}

	private get elementExtractedColumns(): Array<ColumnDefinition<TData, KeyPathOf<TData>>> {
		if (!this.columnsSlot) {
			return []
		}
		...
		return Array.from(assigned ?? children ?? [])
			.filter((c): c is DataGridColumn<TData, KeyPathOf<TData>> => {
	...
	private get autoGeneratedColumnDefinitions() {
		if (!this.dataLength) {
			return []
		}
...
function subDataGridSelectorChanged<TData>(this: DataGrid<TData>) {
	...
	this.getRowDetailsTemplate = (data: TData) => html`
		<mo-data-grid
			...
			headerHidden
			sidePanelHidden
			.columnsDefinitions=${this.columns}
